...
// ── AI Mapmaker ───────────────────────────────────────────────────────────────
const MAPMAKER_SYSTEM_PROMPT = `You are a Constellation map generator. Output ONLY a single valid JSON object — no markdown, no explanation, no code fences.

GAME CONTEXT:
Constellation is a turn-based territory strategy game played on a star graph. Players capture stars and contest HQ nodes to score points.
...
COORDINATE SPACE: x in [0, 319], y in [0, 179]

STAR TYPES:
- 1 = regular capturable star (the vast majority of stars should be type 1)
- 2 = HQ node (high-value headquarters; use 0 for dynamic placement, or an even count: 2, 4, 6...)

STRUCTURAL REQUIREMENTS (MANDATORY — violating any will break the game):
1. Minimum 50 stars (aim for 80-300 for good gameplay)
2. Graph must be CONNECTED — every star must be reachable from every other star via lines
3. avg_degree = (2 x edge_count) / star_count must be >= 2.0
4. Stars must be spread across the full 320x180 canvas — do not cluster everything in one region
5. Line values are 0-based indices into the stars array, so [3, 17] connects stars[3] to stars[17]

MODE AFFINITY GUIDE (use directive to choose structure):
- QTY mode: dense star field, broad canvas coverage, moderate connectivity (avg_degree 2-3)
- SPT mode: clear lanes and bottlenecks, pronounced chokepoints, sparse bridges between clusters
- FRT mode: distinct HQ clusters near opposite canvas edges, contested central territory
- DST mode: HQs placed at extreme opposite corners, long winding paths connecting them
- WRM mode: high connectivity (avg_degree >= 4), mesh-like, many parallel routes between any two points
- INV mode: asymmetric layout, unconventional topology, surprising or counterintuitive structure

Output ONLY the raw JSON object. No markdown fences, no commentary, no explanation.`;

app.post('/api/mapmaker/generate', async (req, res) => {
    const apiKey = process.env.ANTHROPIC_API_KEY;
    if (!apiKey) return res.status(500).json({ error: 'ANTHROPIC_API_KEY not set on server' });

    const { directive, imageBase64, imageMediaType } = req.body;
    ...
    try {
        ...
        if (imageBase64) {
            ...
            userContent.push({
                type: 'text',
                text: `Use this image as spatial and structural inspiration for the star layout — extract shapes, regions, density patterns, or topology from it.\n\nDirective: ${directive.trim()}`
            });
        } else {
            userContent.push({ type: 'text', text: directive.trim() });
        }

        const response = await axios.post('https://api.anthropic.com/v1/messages', {
            model: 'claude-sonnet-4-6',
            max_tokens: 8192,
            system: MAPMAKER_SYSTEM_PROMPT,
        ...
        try {
            ...
            throw new Error('Claude returned non-JSON: ' + rawText.substring(0, 200));
        }

        if (!Array.isArray(map.stars) || !Array.isArray(map.lines)) {
            throw new Error('Invalid map structure: missing stars[] or lines[]');
        }
        if (map.stars.length < 20) {
            throw new Error(`Too few stars: ${map.stars.length} (minimum 20)`);
        }

        const starCount = map.stars.length;
        const edgeCount = map.lines.length;
        const avgDegree = starCount > 0 ? parseFloat((2 * edgeCount / starCount).toFixed(2)) : 0;
        const hqCount = map.stars.filter(s => Array.isArray(s) ? s[2] === 2 : false).length;

        console.log(`🗺  AI map generated: "${map.title}" — ${starCount} stars, ${edgeCount} edges, deg ${avgDegree}, ${hqCount} HQ`);

        res.json({ ok: true, map, stats: { starCount, edgeCount, avgDegree, hqCount } });
    } catch (err) {
        console.error('Mapmaker generate error:', err.message);
        if (err.response?.data) console.error('Claude API:', JSON.stringify(err.response.data).substring(0, 400));
